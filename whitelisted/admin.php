...
// Biztonsági ellenőrzés: csak bejelentkezett adminok láthatják az oldalt
if (!isset($_SESSION['is_admin']) || !$_SESSION['is_admin']) {
    header('Location: index.php');
    exit('Access Denied');
}

// A kérdésekre itt is szükség van a megjelenítéshez
$questions = [
...
// Kérelmek és törlések kezelése
if (isset($_GET['action']) && isset($_GET['id'])) {
    $action = $_GET['action'];
    $user_id = $_GET['id'];

    // Függő kérelmek kezelése (elfogadás/elutasítás)
    ...
    // ÚJ: Whitelisten lévő felhasználó törlése
    if ($action === 'remove_whitelist') {
        if (file_exists($whitelist_file)) {
            ...
            // Újraírjuk a fájlt a frissített tartalommal
...
// Adatok betöltése a megjelenítéshez
$pending_applications = file_exists($pending_file) ? json_decode(file_get_contents($pending_file), true) ?: [] : [];
$pending_applications = array_reverse($pending_applications); // Legújabb elöl
// ÚJ: Whitelisten lévő felhasználók betöltése
